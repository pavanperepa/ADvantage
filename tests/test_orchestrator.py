...
from cricket_posts.campaign.models import CampaignArtifact
from cricket_posts.campaign.orchestrator import OrchestratorError, run_campaign
...
def test_run_campaign_dispatches_reel_requests_to_the_reel_adapter(tmp_path, monkeypatch):
    ...
    def fake_verify_reel(artifact):
        ...
        from cricket_posts.campaign.models import VerificationResult

        return VerificationResult(passed=True, findings=[])

    monkeypatch.setattr("cricket_posts.campaign.orchestrator.produce_reel", fake_produce_reel)
    monkeypatch.setattr("cricket_posts.campaign.orchestrator.verify_reel", fake_verify_reel)
