...
* :func:`build_meta_preview` -- pure, makes no network calls, and derives the
  ...
  the hardcoded constants in ``scripts/meta_ads_create_campaign.py``.
* :func:`create_paused_campaign` -- defaults to a dry run (``execute=False``):
  ...
  (mirroring ``scripts/meta_ads_monitor.py``'s ``fetch()``) to confirm they
...
``scripts/meta_ads_create_campaign.py`` rather than imported from it, for two
concrete reasons:
...
1. ``scripts/`` has no ``__init__.py`` -- it is a folder of standalone CLI
   ...
   from ``src/cricket_posts`` would need ``sys.path`` surgery that would only
...
# advertises from. Same value as scripts/meta_ads_create_campaign.py. There is
...
def upload_video(graph: Graph, path: Path) -> str:
    ...
    Same behavior as ``scripts/meta_ads_create_campaign.py``: in dry-run mode
...
def _build_targeting(request: CampaignRequest) -> dict:
    ...
    ``scripts/meta_ads_create_campaign.py`` found broad targeting (no interest
