...
* ``brand.tagline`` / ``location`` / ``contact_lines`` / ``palette`` are left
  ...
  colour fields to source them from honestly (same reasoning `reel_adapter.py`
...
read here at all (Meta ad prep is `meta_adapter.py`'s job).
...
from ..models import BrandProfile, Contact, ContentSection, InformationContent, PosterContent
from ..pipeline import ComposeResult, PosterComposer
...
def produce_poster(
    request: CampaignRequest, *, workdir: Path
) -> tuple[CampaignArtifact, ComposeResult]:
